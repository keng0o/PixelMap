...
test('preview APIは地図立体感のV3基準橋を使う',()=>{
  assert.equal(preview.version,'pixelmap-bridge-component-preview/3');
});
...
test('URLから5度角度・整数寸法・LOD・背景を読み正規化する',()=>{
  ...
  assert.deepEqual(preview.parseState('?render=bridge-components&angle=no&length=8&width=4&roadWidth=99&detail=bad&background=bad'),{
    angle:45,length:22,masonryWidth:10,roadWidth:2,debug:'none',detail:'medium',background:'water',
  });
});

test('query省略時は45度の幅広い2連アーチ基準形を選ぶ',()=>{
  assert.deepEqual(preview.parseState('?render=bridge-components'),{
...
test('幅変更時も路面を左右欄干の内側へclampする',()=>{
  assert.deepEqual(preview.normalizeState({angle:10,length:52,masonryWidth:16,roadWidth:20,debug:'none'}),{
    angle:10,length:52,masonryWidth:16,roadWidth:8,debug:'none',detail:'medium',background:'water',
  });
});
