...
# Development build QR handoff

- After every feature addition or behavior fix that affects `mobile/`, run `npm run check` from `mobile/`, then run `npm run start:qr` and wait for Expo CLI to print the active development-client URL and QR code.
- Generate the QR code from the active development-client URL for that exact revision. Do not reuse a QR code from an earlier Metro session.
- Keep the Metro server running for the handoff and share both the QR code and its development-client URL. Use LAN by default; if the user's device cannot reach the LAN server, restart with an Expo tunnel and regenerate the QR code.
- A mobile feature change is incomplete until its fresh development build QR code has been generated and shared after validation.
