...
test('standalone testには田中達之風の専用skinと機械素材を残さない',()=>{
  assert.equal('weatheredIndustrialDayZ14' in globalThis.PixelMapWorldStyles,false);
  assert.equal('steampunkMegacityDayZ14' in globalThis.PixelMapWorldStyles,false);
  assert.doesNotMatch(html,/weathered-industrial-day-z14/);
  assert.doesNotMatch(html,/steampunk-megacity-day-z14/);
  assert.doesNotMatch(html,/steampunk-map-assets\.js/);
  assert.doesNotMatch(html,/STANDALONE_WEATHERED_INDUSTRIAL_SKIN|STANDALONE_STEAMPUNK_MEGACITY/);
});
