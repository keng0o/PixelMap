...
test('Canvasは北上固定・名称なし・屋根中心の契約を説明する', () => {
  ...
  assert.match(page, /data-style="top-down-hand-drawn-game-v10"/);
  assert.match(page, /data-bearing="0"/);
});

test('参考素材6巡目のcache versionを全専用moduleへ揃える', () => {
  assert.equal((page.match(/reference-material-6b/g) || []).length, 4);
});
